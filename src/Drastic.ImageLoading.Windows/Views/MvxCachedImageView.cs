...
using Drastic.ImageLoading.Args;
...
namespace Drastic.ImageLoading.Views
{
    public class MvxCachedImageView : ContentControl, ICachedImageView, IDisposable
    {
        ...
        /// <summary>
        /// MvxCachedImageView modded by Drastic Actions
        /// </summary>
        ...
        public event EventHandler<Drastic.ImageLoading.Args.ErrorEventArgs> OnError;
        public event EventHandler<FinishEventArgs> OnFinish;
        public event EventHandler<DownloadStartedEventArgs> OnDownloadStarted;
        public event EventHandler<Drastic.ImageLoading.Args.DownloadProgressEventArgs> OnDownloadProgress;
        ...
        protected virtual void UpdateImageLoadingTask()
        {
            ...
            if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.Url)
            {
                imageLoader = ImageService.Instance.LoadUrl(ffSource.Path, CacheDuration);
            }
            else if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.CompiledResource)
            {
                imageLoader = ImageService.Instance.LoadCompiledResource(ffSource.Path);
            }
            else if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.ApplicationBundle)
            {
                imageLoader = ImageService.Instance.LoadFileFromApplicationBundle(ffSource.Path);
            }
            else if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.Filepath)
            {
                imageLoader = ImageService.Instance.LoadFile(ffSource.Path);
            }
            else if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.Stream)
            {
                imageLoader = ImageService.Instance.LoadStream(ffSource.Stream);
            }
            else if (ffSource.ImageSource == Drastic.ImageLoading.Work.ImageSource.EmbeddedResource)
            {
