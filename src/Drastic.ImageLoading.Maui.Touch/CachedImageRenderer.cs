...
using Foundation;
using Drastic.ImageLoading.Maui;
using Drastic.ImageLoading.Extensions;
using System.Threading.Tasks;
using Drastic.ImageLoading.Helpers;
using Drastic.ImageLoading.Maui.Args;
using Drastic.ImageLoading.Maui.Platform;
...
using PImageView = Drastic.ImageLoading.Maui.Platform.CachedImageRenderer.FormsNSImageView;
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
		...
		/// <summary>
		/// Call this after Microsoft.Maui.Controls.Init to use Drastic.ImageLoading in all Microsoft.Maui.Controls views,
		/// including Microsoft.Maui.Controls.Image
		/// </summary>
		public static void InitImageSourceHandler()
		{
			Helpers.Dependency.Register(typeof(FileImageSource), typeof(FFLoadingImageSourceHandler));
			Helpers.Dependency.Register(typeof(StreamImageSource), typeof(FFLoadingImageSourceHandler));
			Helpers.Dependency.Register(typeof(UriImageSource), typeof(FFLoadingImageSourceHandler));
			Helpers.Dependency.Register(typeof(EmbeddedResourceImageSource), typeof(FFLoadingImageSourceHandler));
			Helpers.Dependency.Register(typeof(DataUrlImageSource), typeof(FFLoadingImageSourceHandler));

			try
			{
				var svgAssembly = Assembly.Load("Drastic.ImageLoading.Svg.Forms");
				if (svgAssembly != null)
				{
					var svgImageSourceType = svgAssembly.GetType("Drastic.ImageLoading.Svg.Forms.SvgImageSource");
					if (svgImageSourceType != null)
					{
						Helpers.Dependency.Register(svgImageSourceType, typeof(FFLoadingImageSourceHandler));
					}
				}
			}
			catch { }
        ...
        private async Task<byte[]> GetImageAsByteAsync(bool usePNG, int quality, int desiredWidth, int desiredHeight)
        {
            ...
            var imageData = usePNG ? image.AsPNG() : image.AsJPEG(new NFloat(quality / 100f));
