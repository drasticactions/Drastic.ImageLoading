...
namespace Drastic.ImageLoading.Extensions
{
    public static class PImageExtensions
    {
        ...
        {
            var widthRatio = desiredWidth / Convert.ToInt64(image.Size.Width);
            var heightRatio = desiredHeight / Convert.ToInt64(image.Size.Height);
            ...
            var aspectWidth = Convert.ToInt64(image.Size.Width) * scaleRatio;
            var aspectHeight = Convert.ToInt64(image.Size.Height) * scaleRatio;
            ...
            try
            {
                image.Draw(new CGRect(new nfloat(0.0), new nfloat(0.0), newSize.Width, newSize.Height));
        ...
        {
#if __IOS__
            return image.AsJPEG(new System.Runtime.InteropServices.NFloat(quality / 100f)).AsStream();
#elif __MACOS__
            // todo: jpeg quality?
            var imageRep = new NSBitmapImageRep(image.AsTiff());
