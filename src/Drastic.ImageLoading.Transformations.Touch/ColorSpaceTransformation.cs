...
namespace Drastic.ImageLoading.Transformations
{
    ...
    public class ColorSpaceTransformation: TransformationBase
    {
        ...
        {
            _colorMatrix.RVector = new CIVector(new nfloat(ma[0][0]), new nfloat(ma[1][0]), new nfloat(ma[2][0]), new nfloat(ma[3][0]));
            _colorMatrix.GVector = new CIVector(new nfloat(ma[0][1]), new nfloat(ma[1][1]), new nfloat(ma[2][1]), new nfloat(ma[3][1]));
            _colorMatrix.BVector = new CIVector(new nfloat(ma[0][2]), new nfloat(ma[1][2]), new nfloat(ma[2][2]), new nfloat(ma[3][2]));
            _colorMatrix.AVector = new CIVector(new nfloat(ma[0][3]), new nfloat(ma[1][3]), new nfloat(ma[2][3]), new nfloat(ma[3][3]));
            _colorMatrix.BiasVector = new CIVector(new nfloat(ma[0][4]), new nfloat(ma[1][4]), new nfloat(ma[2][4]), new nfloat(ma[3][4]));
        }
        ...
        public static UIImage ToColorSpace(UIImage source, CGColorSpace colorSpace)
        {
            ...
            using (var context = new CGBitmapContext(IntPtr.Zero, Convert.ToInt32(bounds.Width), Convert.ToInt32(bounds.Height), cgImage.BitsPerComponent, cgImage.BytesPerRow, colorSpace, CGImageAlphaInfo.None))
            {
